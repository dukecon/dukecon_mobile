...
/**
 * Map a [SpeakerDetailView] to and from a [Speaker] instance when data is moving between this layer
 * and the Domain layer
 */
open class SpeakerDetailMapper constructor(val twitterLinks: TwitterLinks) :
    Mapper<SpeakerDetailView, Speaker> {
  ...
  override fun mapToView(type: Speaker): SpeakerDetailView {
    return SpeakerDetailView(
        ...
        type.title,
        type.twitter,
        twitterLinks.getHandle(type.twitter),
        type.bio,
        type.website,
        type.avatar)
  }
