...
class LibraryViewModel(librariesRepository: LibrariesRepository) : CommonViewModel() {
  ...
  @Suppress("MemberVisibilityCanBePrivate") val libraries: Libraries
    ...
    libraries =
        Libraries(
            librariesRepository
                .getLibraries()
                .filter {
                  (it.targetHost == HostPlatform.COMMON) || (it.targetHost == getTargetHost())
                }
                .map {
                  print(it.targetHost)
                  toLibraryData(it)
  ...
  private fun toLibraryData(it: Library): LibraryData =
      LibraryData(name = it.name, license = it.license, owner = it.owner, url = it.url)
