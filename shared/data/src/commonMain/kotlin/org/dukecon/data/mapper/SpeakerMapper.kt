...
/**
 * Map a [SpeakerModel] to and from a [Speaker] instance when data is moving between this later and
 * the Domain layer
 */
open class SpeakerMapper constructor(private val twitterLinks: TwitterLinks) :
    Mapper<SpeakerEntity, Speaker> {
  ...
  override fun mapFromEntity(type: SpeakerEntity): Speaker {
    return Speaker(
        ...
        type.name,
        type.title,
        twitterLinks.getNormalizedTwitterUrl(type.twitter),
        type.bio,
        type.website,
        type.avatar)
  }
  ...
  override fun mapToEntity(type: Speaker): SpeakerEntity {
    return SpeakerEntity(
        type.id, type.name, type.title, type.twitter, type.bio, type.website, type.avatar)
  }
}
