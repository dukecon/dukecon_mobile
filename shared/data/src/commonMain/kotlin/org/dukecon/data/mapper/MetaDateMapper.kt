...
class MetaDateMapper constructor() : Mapper<MetaDataEntity, MetaData> {
  ...
  private fun mapLocationsFromEntity(locationsEntity: LocationsEntity): Location {
    return Location(
        locationsEntity.id,
        locationsEntity.order,
        locationsEntity.names,
        locationsEntity.icon,
        locationsEntity.capacity)
  }
  ...
  private fun mapLanguageFromEntity(languageEntity: LanguageEntity): Language {
    return Language(
        languageEntity.id,
        languageEntity.code,
        languageEntity.order,
        languageEntity.names,
        languageEntity.icon)
  }

  private fun mapEventTypeFromEntity(eventTypeEntity: EventTypeEntity): EventType {
    return EventType(
        eventTypeEntity.id, eventTypeEntity.order, eventTypeEntity.names, eventTypeEntity.icon)
  }

  private fun mapAudienceFromEntity(audienceEntity: AudienceEntity): Audience {
    return Audience(
        audienceEntity.id, audienceEntity.order, audienceEntity.names, audienceEntity.icon)
  }
