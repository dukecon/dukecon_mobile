 ...
 * data sources
 */
class LocalAndRemoteDataRepository
constructor(
  ...
  override suspend fun getFavorites(): List<Favorite> {
    return localDataStore.getFavorites().map { list -> favoriteMapper.mapFromEntity(list) }
  }
  ...
  override suspend fun getEvent(id: String): Event {
    ...
    return eventMapper.mapFromEntity(
            dataStore.getEvents().first { it.id == id }, speakers, favorites, metaData)
        .also { log(LogLevel.DEBUG, "LocalAndRemoteDataRepository", "getEvent=${id} <==") }
  }
  ...
  override suspend fun getSpeakers(): List<Speaker> {
    val dataStore = localDataStore
    return dataStore.getSpeakers().map { speakerMapper.mapFromEntity(it) }.sortedBy { it.name }
  }
  ...
  override suspend fun getEventDates(): List<GMTDate> {
    ...
    return try {
      ...
      events
          ...
          .also { log(LogLevel.DEBUG, "LocalAndRemoteDataRepository", "getEventDates<==") }
    } catch (e: Exception) {
  ...
  private fun mergeFavorites(
      favorites1: List<FavoriteEntity>,
      favorites2: List<FavoriteEntity>
  ): List<FavoriteEntity> {
  ...
  override suspend fun getEvents(day: Int): List<Event> {
    ...
    return eventEntities
        ...
        .map { eventMapper.mapFromEntity(it, speakers, favorites, metaData) }
        .sortedBy { it.startTime }
        .also {
          log(
              LogLevel.DEBUG,
              "LocalAndRemoteDataRepository",
              "events=${it.size} getEvents=${day} <==")
        }
  }
