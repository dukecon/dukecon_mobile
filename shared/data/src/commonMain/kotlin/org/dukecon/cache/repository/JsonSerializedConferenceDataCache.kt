...
class JsonSerializedConferenceDataCache(
  ...
  init {
    if (json.lastUpadte() > 0) {
      json.run {
        cachedRooms = conference.rooms.map { toRooms(it) }
        cachedEvents = conference.sessions.map { toEvent(it) }
        cacheSpeakers = conference.speakers.map { toSpeaker(it) }
        cacheFavorites = conference.favorites.map { toFavorite(it) }
  ...
  private fun toMetadataModel(it: MetaDataEntity): MetaDataModel =
      MetaDataModel(
          id = it.id, locations = it.locations.map { location -> toLocationModel(location) })
  ...
  private fun toFavorite(it: FavoriteModel): FavoriteEntity {
    return FavoriteEntity(id = it.id, version = it.version)
  }
  ...
  private fun toRooms(room: RoomModel): RoomEntity {
    return RoomEntity(id = room.id, name = room.name)
  }
  ...
  private fun toSessionModel(event: EventEntity): EventModel =
      EventModel(
          id = event.id,
          title = event.title,
  ...
  private fun toIsoTime(startTime: GMTDate): String {
    return "${startTime.year}-${twoDigits(startTime.month.ordinal + 1)}-${twoDigits(startTime.dayOfMonth)}T${
            twoDigits(
                startTime.hours
            )
        }:${twoDigits(startTime.minutes)}:${twoDigits(startTime.seconds)}"
  }
  ...
  override fun saveSpeakers(speakers: List<SpeakerEntity>) {
    ...
    json.conference =
        json.conference.copy(speakers = speakers.map { speaker -> toSpeakerModel(speaker) })
  }

  private fun toSpeakerModel(speaker: SpeakerEntity): SpeakerModel =
      SpeakerModel(
          id = speaker.id,
          name = speaker.name,
          title = speaker.title,
  ...
  private fun toRoomsModel(room: RoomEntity): RoomModel = RoomModel(id = room.id, name = room.name)
}
