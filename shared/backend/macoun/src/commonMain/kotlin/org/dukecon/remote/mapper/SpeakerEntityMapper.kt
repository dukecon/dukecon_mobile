package org.dukecon.remote.mapper

import org.dukecon.aspects.twitter.TwitterLinks
import org.dukecon.data.model.SpeakerEntity
import org.dukecon.data.source.ConferenceConfiguration
import org.dukecon.remote.models.Speaker

/**
 * Map a [Speaker] to and from a [SpeakerEntity] instance when data is moving between this later and
 * the Data layer
 */
internal open class SpeakerEntityMapper
constructor(
    private val conferenceConfiguration: ConferenceConfiguration,
    private val twitterLinkMapper: TwitterLinks
) : EntityMapper<Speaker, SpeakerEntity> {

  /** Map an instance of a [Speaker] to a [SpeakerEntity] model */
  override fun mapFromRemote(type: Speaker): SpeakerEntity {
    return SpeakerEntity(
        id = type.id ?: "",
        name = type.firstname ?: "" + " " + type.lastname,
        twitter = twitterLinkMapper.getNormalizedTwitterUrl(type.twitter ?: ""),
        avatar = getAvatarUrlFromId(type.image ?: ""),
        bio = type.description ?: "",
        title = "",
        website = "")
  }

  private fun getAvatarUrlFromId(s: String): String {
    return conferenceConfiguration.speakerAvatarUrl + s
  }
}
