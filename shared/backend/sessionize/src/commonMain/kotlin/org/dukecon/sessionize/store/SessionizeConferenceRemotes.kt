...
/**
 * Sessionize backed network calls remote service. Makes netowork calls and convert data to
 * RemoteData Store DTO.
 */
class SessionizeConferenceRemotes(private val sessionizeApi: SessionizeApi) : ConferenceRemote {
  override suspend fun getSpeakers(): List<SpeakerEntity> {
    try {
      return sessionizeApi.getSpeakers().map { toSpeakerEntity(it) }
  ...
  private fun toSpeakerEntity(it: Speaker): SpeakerEntity =
      SpeakerEntity(
          id = it.id ?: "",
          name = it.fullName,
          title = it.tagLine,
  ...
  override suspend fun getRooms(): List<RoomEntity> {
    try {
      return sessionizeApi.getRooms().map { toRoomEntity(it) }
    } catch (source: Throwable) {
      println(source.toString())
    }
    return emptyList()
  ...
  private fun toRoomEntity(it: Room): RoomEntity = RoomEntity(id = it.id, name = it.name)
  ...
  private fun toLocationsEntity(it: RoomEntity): LocationsEntity =
      LocationsEntity(
          id = it.id, order = 0, names = mapOf(Pair("de", it.name)), icon = "", capacity = 0)
  ...
  override suspend fun getEvents(): List<EventEntity> {
    try {
      ...
      return sessionizeApi.getSessions().map { toEventEntity(it, rooms) }
    } catch (source: Throwable) {
      println(source.toString())
    }
    return emptyList()
  ...
  private fun toEventEntity(it: Session, rooms: List<Room>): EventEntity =
      EventEntity(
          id = it.id ?: "",
          title = it.title ?: "",
