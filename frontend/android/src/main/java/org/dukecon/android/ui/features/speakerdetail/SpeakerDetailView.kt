...
class SpeakerDetailView(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) :
  ...
  override fun showSpeaker(speaker: org.dukecon.presentation.model.SpeakerDetailView) {
    ...
    if (speaker.twitterHandle.isNotEmpty()) {
      val twitterIntent = Intent(Intent.ACTION_VIEW).apply { data = Uri.parse(speaker.twitter) }
      ...
      if (twitterIntent.resolveActivity(context.getPackageManager()) != null) {
        ...
        twitter.setCompoundDrawablesWithIntrinsicBounds(
            DrawableUtils.create(context, R.drawable.ic_logo_twitter), null, null, null)

        twitter.setOnClickListener { context.startActivity(twitterIntent) }
      }
    } else {
    ...
    if (speaker.website.isNotEmpty()) {
      ...
      github.setCompoundDrawablesWithIntrinsicBounds(
          DrawableUtils.create(context, R.drawable.ic_logo_github), null, null, null)

      github.setOnClickListener {
        val githubIntent = Intent(Intent.ACTION_VIEW).apply { data = Uri.parse(speaker.website) }
    ...
    image.apply { load(speaker.avatar) { placeholder(R.drawable.ph_speaker) } }
  }
}
