  ...
  override fun onBindViewHolder(holder: ViewHolder, position: Int) {
    ...
    if (session.speakers.isEmpty()) {
      ...
      holder.room.setCompoundDrawablesWithIntrinsicBounds(
          DrawableUtils.create(context, R.drawable.ic_speaker), null, null, null)
    }
    ...
    if (session.room.isNotEmpty()) {
      val duration =
          String.format(
              ...
              Duration.inMinutes(session.startTime, session.endTime))
      holder.room.visibility = View.VISIBLE
      holder.room.text =
          context.getString(R.string.event_list_room_duration, session.room, duration)
      holder.room.setCompoundDrawablesWithIntrinsicBounds(
          DrawableUtils.create(context, R.drawable.ic_room), null, null, null)
    } else {
    ...
    if (position > 0) {
      ...
      holder.timeslot.visibility =
          if (previous.startTime == session.startTime) {
            View.INVISIBLE
          } else {
            View.VISIBLE
          }
