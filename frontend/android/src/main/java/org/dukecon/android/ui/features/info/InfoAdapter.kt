...
class InfoAdapter(private val libraryListener: (library: LibraryView) -> Unit) :
  ...
  fun setLibraries(libraries: List<LibraryView>) {
    ...
    for (library in libraries) {
      val libs =
          if (this.libraries.contains(library.owner)) {
  ...
  class ViewHolder(itemView: View, val libraryListener: (library: LibraryView) -> Unit) :
