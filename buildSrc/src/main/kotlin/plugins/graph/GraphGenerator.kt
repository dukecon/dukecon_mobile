...
// Based on
// https://github.com/JakeWharton/SdkSearch/blob/766d612ed52cdf3af9cd0728b6afd87006746ae5/gradle/projectDependencyGraph.gradle
internal class ProjectDependencyGraphGenerator(
    ...
    private val projectGenerator: DependencyGraphGeneratorExtension.ProjectGenerator =
        DependencyGraphGeneratorExtension.ProjectGenerator()
) {
  fun generateGraph(): MutableGraph {
    val graph = mutGraph().setDirected(true)
    graph
        .graphAttrs()
        .add(Label.of(project.name).locate(Label.Location.TOP), Font.size(DEFAULT_FONT_SIZE))
  ...
  private fun addProjects(
      ...
      rootProjects: MutableList<Project>,
        ...
        .flatMap { (project, configuration) ->
          configuration
              ...
              .flatMap { projectDependency ->
                ...
                dependencies.add(
                    ProjectDependencyContainer(
                        project,
                        projectDependency,
                        configuration.name.toLowerCase(US).endsWith("implementation")))
  ...
  private fun rankRootProjects(
      graph: MutableGraph,
      projects: MutableSet<Project>,
      rootProjects: MutableList<Project>
  ) {
    // graph.add(graph()                .graphAttr()
    // .with(Rank().SAME)
    // .with(projects.filter { rootProjects.contains(it) }.map { mutNode(it.path) }.toList())
    // )
  }
  ...
  private fun addDependencies(
    ...
    dependencies
        //             .filterNot { (from, to, _) -> !from.isCommonsProject() &&
        // to.isCommonsProject() }
